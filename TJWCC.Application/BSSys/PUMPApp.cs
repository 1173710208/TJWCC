...
namespace TJWCC.Application.BSSys
{
    ...
    {
        private IPUMPRepository service = new PUMPRepository();

        /// <summary>
        /// 获取SCADA
        /// </summary>
        /// <returns></returns>
        public List<PUMPEntity> GetList()
        {
            return service.IQueryable().Where(item => item.Is_Active == 1).ToList();
        }

        public List<PUMPEntity> GetList(int elementId)
        {
            return service.IQueryable().Where(i => i.ElementId == elementId && i.Is_Active == 1).ToList();
        }
        /// <summary>
        /// 分页查询SCADA数据记录
        /// </summary>
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public List<PUMPEntity> GetList(Pagination pagination, int id)
        {
            var expression = ExtLinq.True<PUMPEntity>();
        ...
        public void SubmitForm(PUMPEntity moduleEntity, string id)
        {
