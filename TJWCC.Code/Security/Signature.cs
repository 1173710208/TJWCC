...
namespace TJWCC.Code
{
    public class Signature
    {
        ...
        /// <returns></returns>
        public static string GenerateSignature(String token, string nonce, string msg)
        {
            //token长度 + 8B随机字符串长度 + 消息长度 
            using (MD5 md5Hash = MD5.Create())
            {
                var source = token + nonce + msg;
                return CryptoUtil.GetMd5Hash(md5Hash, source);
            }
        }
        ...
        public static BodyObj resolveBody(string body, bool encrypted)
        {
            ...
            obj.msgSignature = jsonMsg.GetValue("signature").ToString();
            if (encrypted)
            {
