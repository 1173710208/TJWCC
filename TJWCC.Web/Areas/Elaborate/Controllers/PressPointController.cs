...
{
    public class PressPointController : ControllerBase
    {
        private BSM_Meter_InfoApp bmiApp = new BSM_Meter_InfoApp();
        private JUNCTIONApp junApp = new JUNCTIONApp();
        string pointList = "";
        // 压力监测点优化布置: Elaborate/PressPoint

        public ActionResult GetPressPointList()
        {
            var result = bmiApp.GetPointList(2);
            return Content(result.ToJson());
        }
        public ActionResult CreateNewPressPoint(int? isArea, int pCount, string areaids)
        {
            var result = bmiApp.GetPointList(2);
            string sArguments = @"Pressure.py";//这里是python的文件名字
            string strArr = pCount.ToString();
            if (isArea == null)
                RunPythonScript(sArguments, strArr, "-u");
            else
                RunPythonScript(sArguments, strArr, "-u", isArea.Value, areaids);
            return Content(pointList);
        }
        //调用python核心代码
        public void RunPythonScript(string sArgName, string teps, string args = "", int isArea = 1, string areaids = "1" )
        {
            string nowDate = DateTime.Now.ToString("yyyyMMddHHmmss");
            StreamWriter sw = System.IO.File.AppendText("D:\\sambo\\data\\areaids" + nowDate + ".txt");
            sw.Write(areaids);
            sw.Flush();
            sw.Close();
            ...
            sArguments += " " + isArea + " " + teps + " " + nowDate;//传递参数
            sArguments += " " + args;
        ...
        public void p_OutputDataReceived(object sender, DataReceivedEventArgs e)
        {
            ...
            {
                pointList = e.Data.Substring(1, e.Data.Length - 2).Replace(" ", "").Replace("'", "");
            }
        }
        public ActionResult CreatePressOrder(string ids)
        {
        ...
        {
            string path = junApp.GetDownload(ids);
            return Content(path);
        }
    }
}