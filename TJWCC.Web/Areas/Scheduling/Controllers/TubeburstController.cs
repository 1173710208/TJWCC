...
{
    public class TubeburstController : ControllerBase
    {
        ...
        [HttpPost]
        public JsonResult TubeBurstAnalysis(int elementId,int nodeType,string cnbValves)
        {
            ...
            //cmd.CommandText = "UPDATE Table_A SET Table_A.Result_Pressure_5 = Table_B.Result_Pressure_24 FROM RESULT_JUNCTION_TJWCC AS Table_A INNER JOIN RESULT_JUNCTION AS Table_B " +
            //    "ON Table_A.ElementId = Table_B.ElementId WHERE Table_A.Result_Pressure_5 <= 0.01";
            //try
            //{
            //    cmd.ExecuteNonQuery();
            //    db.Database.Connection.Close();
            ...
            //string path = AppDomain.CurrentDomain.SetupInformation.ApplicationBase + "Tubeburst.py";// 获得python文件的绝对路径（将文件放在c#的debug文件夹中可以这样操作）
            //path = @"D:\sambo\Tubeburst.py";//(因为我没放debug下，所以直接写的绝对路径,替换掉上面的路径了)
            ...
            //sArguments += " " + foundValves + " " + foundNodes;//传递参数
            //sArguments += " -u";
        ...
        public ActionResult CreatePlan(string valveList, string base64Info)
        {
            ...
            document.LastSection.AddParagraph("关阀后受影响用户明细表(以地表计,共" + items3.Count + "块)", "Heading2");
            ...
            var ndp = dpApp.GetNewPlan(4);
            //result.Add(JObject.FromObject(ndp));
            result.Add(path);
            result.Add(path);
            return Content(result.ToJson());
        }
        public ActionResult CreateTubOrder(int? planId)
        {
