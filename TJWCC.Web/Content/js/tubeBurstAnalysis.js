...
$(function () {
    $("#resultDivBoxCalBut").click(function () {//爆管分析计算结果重新计算按钮单击事件
        ...
        tubeBurstAnalysis(minFlowPT, nodeId, nodeType0, cnbValves);
        $.loading('正在计算中...');
    });
...
function tubeBurstAnalysisClick(th) {//
    if (isTubeBurst) {
        recoverAll();
        hideAndClear();
        ...
        setValueForDiv("followMouseInfo", "双击地图计算爆管分析")
...
function tubeBurstAnalysis(pt, elementId, elementTypeId, canNotBeclosedValves) {
    ...
    $.ajax({
        ...
        success: function (data, response, status) {
            ...
            if (data.length > 0) {
                ...
                right622_data = "<tr class='right6221'><td class='right62211'>" + influence_user[0][1] + "</td ><td class='right62211'>" + (influence_user[0][1] * 400 / 1000000) + "</td></tr>";//影响用户信息
                $("#right1").html("关阀后受影响用户明细表(以地表计,共" + influence_user[0][1] + "块)");
                valveList1 = influence_user[0][1] + "," + (influence_user[0][1] * 400 / 1000000);
                ...
                for (var i = 0; i < _foundPipes.length; i++) {//管道高亮处理
                    ...
                    queryTask1.execute(query1, function (featureSet) {
                        ...
                        if (featureSet.features.length > 0) {
                            isChangShow = true;
                            graphicslayer.add(featureSet.features[0].setSymbol(tuoutline).setInfoTemplate(infoTemplate1));
        ...
        complete: function () {
            ...
            $('#resultcloseValvesDiv').css("display", "none");//隐藏关键阀门信息显示
            $('#resultDiv').css("display", "");//显示爆管分析结果列表
            $('#showTA').removeClass('disabled');
            $("#showTA").attr('disabled', false);
